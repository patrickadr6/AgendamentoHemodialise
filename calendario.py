...
def separar_segunda_a_sexta(mes, ano):
    dias_por_mes = calendar.monthrange(ano, mes)[1]
    lista_dias = [datetime.date(ano, mes, dia) for dia in range(1, dias_por_mes + 1)]
    ...
    for dia in lista_dias:
        if dia.weekday() == 0:
            segunda_a_sexta[dia.strftime('%d/%m/%Y')] = 'seg'
        elif dia.weekday() == 1:
            segunda_a_sexta[dia.strftime('%d/%m/%Y')] = 'ter'
        elif dia.weekday() == 2:
            segunda_a_sexta[dia.strftime('%d/%m/%Y')] = 'qua'
        elif dia.weekday() == 3:
            segunda_a_sexta[dia.strftime('%d/%m/%Y')] = 'qui'
        elif dia.weekday() == 4:
            segunda_a_sexta[dia.strftime('%d/%m/%Y')] = 'sex'
